...
func runServer(cmd *cobra.Command, args []string) {
	logger.Logger.Debug().Msg("Brain server starting...")
	...
	brainServer, err := server.NewBrainServer(cfg)
	if err != nil {
	...
	fmt.Printf("Brain server running on http://localhost:%d\n", cfg.Brain.Port)
...
func loadConfig(configFile string) (*config.Config, error) {
	...
	if configFile != "" {
		...
		if envConfigFile := os.Getenv("BRAIN_CONFIG_FILE"); envConfigFile != "" {
			...
			viper.AddConfigPath(filepath.Join(homeDir, ".brain"))
	...
	if cfg.UI.SourcePath == "" {
		candidateSourcePaths := []string{
			...
			filepath.Join(filepath.Dir(execDir), "apps", "platform", "brain", "web", "client"),
			filepath.Join("apps", "platform", "brain", "web", "client"),
	...
	if cfg.UI.DistPath == "" {
		candidateDistPaths := []string{
			...
			filepath.Join(filepath.Dir(execDir), "apps", "platform", "brain", "web", "client", "dist"),
			filepath.Join("apps", "platform", "brain", "web", "client", "dist"),
	...
	fmt.Printf("Loaded config - Storage mode: %s, Brain Port: %d, UI Mode: %s, UI Enabled: %t\n",
		cfg.Storage.Mode, cfg.Brain.Port, cfg.UI.Mode, cfg.UI.Enabled)
