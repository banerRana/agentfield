...
import (
	"github.com/your-org/brain/control-plane/internal/cli"
	...
	"github.com/your-org/brain/control-plane/web/client"
	"fmt"
	"log"
	"os"
	"os/exec"
...
var (
	...
	newBrainServerFunc   = server.NewBrainServer
	buildUIFunc          = buildUI
	...
	startBrainServerFunc = defaultStartBrainServer
)
...
func runServer(cmd *cobra.Command, args []string) {
	fmt.Println("Brain server starting...")
	...
	if envPort := os.Getenv("BRAIN_PORT"); envPort != "" {
	...
	if !storageModeExplicit {
		if envMode := os.Getenv("BRAIN_STORAGE_MODE"); envMode != "" {
	...
	if postgresURL == "" {
		if env := os.Getenv("BRAIN_POSTGRES_URL"); env != "" {
			postgresURL = env
		} else if env := os.Getenv("BRAIN_STORAGE_POSTGRES_URL"); env != "" {
			postgresURL = env
		}
	}
	...
	if env := os.Getenv("BRAIN_STORAGE_POSTGRES_HOST"); env != "" {
		cfg.Storage.Postgres.Host = env
	}
	if env := os.Getenv("BRAIN_STORAGE_POSTGRES_PORT"); env != "" {
		if port, err := strconv.Atoi(env); err == nil {
			cfg.Storage.Postgres.Port = port
		}
	}
	if env := os.Getenv("BRAIN_STORAGE_POSTGRES_DATABASE"); env != "" {
		cfg.Storage.Postgres.Database = env
	}
	if env := os.Getenv("BRAIN_STORAGE_POSTGRES_USER"); env != "" {
		cfg.Storage.Postgres.User = env
	}
	if env := os.Getenv("BRAIN_STORAGE_POSTGRES_PASSWORD"); env != "" {
		cfg.Storage.Postgres.Password = env
	}
	if env := os.Getenv("BRAIN_STORAGE_POSTGRES_SSLMODE"); env != "" {
	...
	// Create Brain server instance
	brainServer, err := newBrainServerFunc(cfg)
	if err != nil {
		log.Fatalf("Failed to create Brain server: %v", err)
	}

	// Start the server in a goroutine so we can open the browser
	go func() {
		fmt.Printf("Brain server attempting to start on port %d...\n", cfg.Brain.Port)
		if err := startBrainServerFunc(brainServer); err != nil {
			log.Fatalf("Failed to start Brain server: %v", err)
		}
	}()
	...
	if cfg.UI.Enabled && openBrowserFlag && !backendOnly {
		uiTargetURL := fmt.Sprintf("http://localhost:%d", cfg.Brain.Port)
		if cfg.UI.Mode == "dev" {
	...
	fmt.Printf("Brain server running. Press Ctrl+C to exit.\n")
...
func loadConfig(configFile string) (*config.Config, error) {
	...
	if configFile != "/dev/null" && configFile != "" {
		...
		if envConfigFile := os.Getenv("BRAIN_CONFIG_FILE"); envConfigFile != "" {
			viper.SetConfigFile(envConfigFile)
		} else {
			viper.SetConfigName("brain")    // name of config file (without extension)
	...
	if cfg.Brain.Port == 0 {
		cfg.Brain.Port = 8080 // Default port
	}
	if cfg.Storage.Mode == "" {
	...
	if cfg.UI.SourcePath == "" {
		...
		if err != nil {
			cfg.UI.SourcePath = filepath.Join("apps", "platform", "brain", "web", "client")
			...
			// This assumes the binary is built in the brain/ directory
			cfg.UI.SourcePath = filepath.Join(execDir, "web", "client")

			// If that doesn't exist, try going up one level (if binary is in brain/)
			if _, err := os.Stat(cfg.UI.SourcePath); os.IsNotExist(err) {
				cfg.UI.SourcePath = filepath.Join(filepath.Dir(execDir), "apps", "platform", "brain", "web", "client")
			}

			// Final fallback to current working directory
			if _, err := os.Stat(cfg.UI.SourcePath); os.IsNotExist(err) {
				altPath := filepath.Join("apps", "platform", "brain", "web", "client")
	...
	if cfg.Storage.Mode == "local" {
		...
		// Ensure all Brain data directories exist
		if _, err := utils.EnsureDataDirectories(); err != nil {
			return nil, fmt.Errorf("failed to create Brain data directories: %w", err)
		}
	}

	fmt.Printf("Loaded config - Storage mode: %s, Brain Port: %d, UI Mode: %s, UI Enabled: %t, DID Enabled: %t\n",
		cfg.Storage.Mode, cfg.Brain.Port, cfg.UI.Mode, cfg.UI.Enabled, cfg.Features.DID.Enabled)
...
func buildUI(cfg *config.Config) error {
	...
	buildEnv = append(buildEnv, fmt.Sprintf("VITE_API_PROXY_TARGET=http://localhost:%d", cfg.Brain.Port))
...
func defaultStartBrainServer(s *server.BrainServer) error {
	return s.Start()
}
