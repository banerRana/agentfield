...
func NewConfigCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Configure environment variables for an installed Brain agent package",
		Long: `Configure environment variables for an installed Brain agent package.
...
Examples:
  brain config my-agent                    # Interactive configuration
  brain config my-agent --list            # List current configuration
  brain config my-agent --set KEY=VALUE   # Set specific variable
  brain config my-agent --unset KEY       # Remove variable`,
...
func (pcm *PackageConfigManager) InteractiveConfig(packageName string) error {
	...
	fmt.Printf("💡 Run 'brain run %s' to start the agent with these settings\n", packageName)
...
func (pcm *PackageConfigManager) loadPackageMetadata(packageName string) (*packages.PackageMetadata, string, error) {
	...
	registryPath := filepath.Join(pcm.BrainHome, "installed.yaml")
	...
	// Load package metadata
	metadataPath := filepath.Join(installedPackage.Path, "brain-package.yaml")
...
func (pcm *PackageConfigManager) saveEnvFile(packagePath string, envVars map[string]string) error {
	...
	var lines []string
	lines = append(lines, "# Environment variables for Brain agent")
	lines = append(lines, "# Generated by 'brain config' command")
	lines = append(lines, "")
